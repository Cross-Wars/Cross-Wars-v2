...
function make2DArray(data) {
  const array = data.gridnums
  const divisor = Math.sqrt(array.length)
  let result = []
  for (let i = 0; i < divisor; i++) {
    result.push(array.slice(i * divisor, (i + 1) * divisor))
  }
  return result
}
...
const dataFormatter = function (data) {
  const across = {}
  const down = {}
  let i = 0
